...
{
    // el deslizamiento comienza después de un retraso y se detiene después de un retraso. necesitamos una enumeración.
    public enum SlidingState : byte { NONE, STARTING, SLIDING, STOPPING };

    //  Controlador de caracteres abierto. Maneja el movimiento de un personaje, mediante el uso de una cápsula para la detección de movimiento y colisión.
    // Nota: la cápsula siempre está en posición vertical. Ignora la rotación.
    [RequireComponent(typeof(CapsuleCollider))]
    [RequireComponent(typeof(Rigidbody))]
    public class CharacterController2k : MonoBehaviour
    {
        // Disparado en colisión con colisionadores en el mundo
        public event Action<CollisionInfo> collision;

        [Header("Player Root")]
        [FormerlySerializedAs("m_PlayerRootTransform")]
        [Tooltip("El hueso raíz del avatar.")]
        public Transform playerRootTransform;

        [FormerlySerializedAs("m_RootTransformOffset")]
        [Tooltip("La transformación raíz se colocará en este desplazamiento")]
        public Vector3 rootTransformOffset = Vector3.zero;

        [Header("Collision")]
        [FormerlySerializedAs("m_SlopeLimit")]
        [Tooltip("Limita el colisionador a subir solo pendientes que son menos empinadas (en grados) que el valor indicado")]
        public float slopeLimit = 45.0f;

        [FormerlySerializedAs("m_StepOffset")]
        [Tooltip("El personaje subirá una escalera solo si está más cerca del suelo que el valor indicado" +
                 "Esto no debe ser mayor que la altura del Controlador de personaje o generará un error" +
                 "En general, esto debe mantenerse lo más pequeño posible")]
        public float stepOffset = 0.3f;

        [FormerlySerializedAs("m_SkinWidth")]
        [Tooltip("Dos colisionadores pueden penetrar entre sí tan profundamente como su Ancho de piel. Los anchos de piel más grandes reducen la vibración" +
                 "El ancho de piel bajo puede hacer que el personaje se atasque. Una buena configuración es hacer que este valor sea el 10% del radio")]
        public float skinWidth = 0.08f;

        [FormerlySerializedAs("m_GroundedTestDistance")]
        [Tooltip("Distancia a probar debajo del personaje cuando se realiza la prueba de conexión a tierra. Aumente si controller.isGunded no da los resultados correctos o cambia entre verdadero / falso mucho")]
        public float groundedTestDistance = 0.002f; // 0.001f no es suficiente para grandes BoxColliders como Floor de uSurvival, aunque funcionaría para MeshColliders.

        [FormerlySerializedAs("m_Center")]
        [Tooltip("Esto compensará el Capsule Collider en el espacio mundial y no afectará la forma en que el personaje gira" +
                 "Idealmente, xyz deberían ser cero para evitar girar hacia otro colisionador")]
        public Vector3 center;

        [FormerlySerializedAs("m_Radius")]
        [Tooltip("Longitud del radio del colisionador de cápsulas. Este es básicamente el ancho del colisionador.")]
        public float radius = 0.5f;

        [FormerlySerializedAs("m_Height")]
        [Tooltip("La altura del Colisionador de cápsulas del personaje. Debe ser al menos el doble del radio")]
        public float height = 2.0f;

        [FormerlySerializedAs("m_CollisionLayerMask")]
        [Tooltip("Capas con las que realizar pruebas de colisiones")]
        public LayerMask collisionLayerMask = ~0; // ~0 sets it to Everything

        [FormerlySerializedAs("m_IsLocalHuman")]
        [Tooltip("¿El personaje está controlado por un humano local? Si es cierto, se realizan más cálculos para un movimiento más preciso")]
        public bool isLocalHuman = true;

        [FormerlySerializedAs("m_SlideAlongCeiling")]
        [Tooltip("¿Puede el personaje deslizarse verticalmente al tocar el techo ? (Por ejemplo, si el techo está inclinado)")]
        public bool slideAlongCeiling = true;

        [FormerlySerializedAs("m_SlowAgainstWalls")]
        [Tooltip("¿Debería el personaje reducir la velocidad contra las paredes?")]
        public bool slowAgainstWalls = false;

        [FormerlySerializedAs("m_MinSlowAgainstWallsAngle")]
        [Range(0.0f, 90.0f), Tooltip("El ángulo mínimo desde el cual el personaje comenzará a disminuir la velocidad en las paredes.")]
        public float minSlowAgainstWallsAngle = 10.0f;

        [FormerlySerializedAs("m_TriggerQuery")]
        [Tooltip("La interacción deseada que deben realizar las llamadas de transmisión con los activadores")]
        public QueryTriggerInteraction triggerQuery = QueryTriggerInteraction.Ignore;

        [Header("Sliding")]
        [FormerlySerializedAs("m_SlideDownSlopes")]
        [Tooltip("¿Debería el personaje deslizarse cuesta abajo cuando su ángulo es mayor que el límite de la pendiente?")]
        public bool slideDownSlopes = true;

        [FormerlySerializedAs("m_SlideMaxSpeed")]
        [Tooltip("La velocidad máxima a la que el personaje puede deslizarse hacia abajo")]
        public float slideMaxSpeed = 10.0f;

        [FormerlySerializedAs("m_SlideGravityScale")]
        [Tooltip("Multiplicador de gravedad para aplicar al deslizarse por pendientes")]
        public float slideGravityMultiplier = 1.0f;

        [FormerlySerializedAs("m_SlideStartTime")]
        [Tooltip("El tiempo (en segundos) después de iniciar una diapositiva clasificada como inicio de diapositiva. Se usa para inhabilitar el salto")]
        public float slideStartDelay = 0.1f;

        [Tooltip("Ligero retraso (en segundos) antes de que dejemos de deslizarnos por pendientes. Para manejar casos en los que la prueba de deslizamiento falla durante algunos fotogramas")]
        public float slideStopDelay = 0.1f;

        // Límite máximo de pendiente.
        const float k_MaxSlopeLimit = 90.0f;

        // Ángulo de pendiente máximo en el que el personaje puede deslizarse hacia abajo automáticamente.
        const float k_MaxSlideAngle = 90.0f;

        // Distancia para probar el suelo al deslizarse por pendientes.
        const float k_SlideDownSlopeTestDistance = 1.0f;

        // Distancia para alejarse de las pendientes al deslizarse por ellas.
        const float k_PushAwayFromSlopeDistance = 0.001f;

        // Distancia mínima que se debe usar cuando se mira hacia adelante para ver si hay pendientes pronunciadas, cuando se verifica si es seguro hacer el desplazamiento de paso.
        const float k_MinCheckSteepSlopeAheadDistance = 0.2f;

        // Ancho mínimo de piel.
        const float k_MinSkinWidth = 0.0001f;

        // Las iteraciones de movimiento máximas. Se utiliza principalmente como a prueba de fallas para evitar un bucle infinito.
        const int k_MaxMoveIterations = 20;

        // Quédate en el suelo si está a menos de esta distancia del personaje.
        const float k_MaxStickToGroundDownDistance = 1.0f;

        // Distancia mínima para probar el suelo cuando se pega al suelo.
        const float k_MinStickToGroundDownDistance = 0.01f;

        // Máx colisionadores para usar en los métodos de superposición.
        const int k_MaxOverlapColliders = 10;

        // Desplazamiento para usar cuando se mueve a un punto de colisión, para tratar de evitar la superposición de los colisionadores
        const float k_CollisionOffset = 0.001f;

        // Distancia mínima para moverse. Esto minimiza pequeñas penetraciones y moldes inexactos (por ejemplo, en el piso)
        const float k_MinMoveDistance = 0.0001f;

        // Altura mínima de desplazamiento de paso para mover (si el personaje tiene un desplazamiento de paso).
        const float k_MinStepOffsetHeight = k_MinMoveDistance;

        // Valor pequeño para probar si el vector de movimiento es pequeño.
        const float k_SmallMoveVector = 1e-6f;

        // Si el ángulo entre la emisión de rayos y el molde de cápsula / esfera normal es menor que esto, utilice la emisión de rayos normal, que es más precisa.
        const float k_MaxAngleToUseRaycastNormal = 5.0f;

        // Escale la distancia de impacto de la cápsula / esfera al hacer el lanzamiento de rayos adicional para obtener una normalidad más precisa
        const float k_RaycastScaleDistance = 2.0f;

        // La comprobación de pendiente por delante está limitada por la distancia movida multiplicada por esta escala.// Slope check ahead is clamped by the distance moved multiplied by this scale.
        const float k_SlopeCheckDistanceMultiplier = 5.0f;

        // El colisionador de cápsulas.
        CapsuleCollider m_CapsuleCollider;

        // La posición al inicio del movimiento.
        Vector3 m_StartPosition;

        // Vectores de movimiento utilizados en el bucle de movimiento.
        List<MoveVector> m_MoveVectors = new List<MoveVector>();

        // Siguiente índice en la lista moveVectors.
        int m_NextMoveVectorIndex;


        // Superficie normal de la última colisión mientras se baja
