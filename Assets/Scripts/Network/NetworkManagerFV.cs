...
public class NetworkManagerFV : NetworkManager
{
    ...
    [Header("Database")]
    public int characterLimit =2; //Total de avatares para crear
    ...
    {
        Debug.Log("OnServerConnect del NetworkManager");
    ...
    public Transform GetStartPositionFor(string classCharacter)
    {
        ...
        foreach (Transform startPosition in startPositions)
        {
            ...
            if (spawn != null &&
                ...
                spawn.playerPrefab.name == classCharacter)
    ...
    Player CreateCharacter(GameObject classPrefab, string characterName, string account, bool administrator)
    {
        ...
        player.isAdministrator = administrator;
                ...
                if (!Database.CharacterExists(message.name))
                {
                    ...
                    if (Database.CharactersForAccount(account).Count < characterLimit)
                    {
                        ...
                        if (0 <= message.classIndex && message.classIndex < playerClasses.Count)
                        {
                            ...
                            if (message.administrator == false ||
                                conn == NetworkServer.localConnection)
                            {
                                // create new character based on the prefab.
                                Player player = CreateCharacter(playerClasses[message.classIndex].gameObject, message.name, account, message.administrator);
    ...
    // player saving ///////////////////////////////////////////////////////////
    // Si invocáramos una función de guardado cada pocos minutos en cada jugador por separado, 
    // podría ocurrir que dos jugadores intercambien objetos y sólo uno de ellos se guarde antes 
    // de que el servidor se caiga, causando así duplicados de objetos.
    void SavePlayers()
    {
        Database.CharacterSaveMany(Player.onlinePlayers.Values); //-------------------------------------------------------
