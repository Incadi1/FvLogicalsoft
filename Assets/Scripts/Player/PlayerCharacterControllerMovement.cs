...
#pragma warning restore CS0109 // member does not hide accessible member
    ...
    [Header("Walking")]
    float walkSpeed = 8; // not public, we use Entity's speed via SetSpeed
    [Range(0f, 1f)] public float walkStepLength = 0.7f;
    public float walkStepInterval = 3;
    public float walkCycleLegOffset = 0.2f; //specific to the character in sample assets, will need to be modified to work with others
    float stepCycle;
    float nextStep;
    [Tooltip("Option to automatically walk forward while both mouse buttons are pressed. This can be useful for long stretches of walking.")]
    public bool walkWhileBothMouseButtonsPressed = true;
    ...
    Vector2 GetInputDirection()
    {
        ...
        if (walkWhileBothMouseButtonsPressed &&
    ...
    MoveState UpdateWALKING(Vector2 inputDir, Vector3 desiredDir)
    {
        // QE key rotation
        if (player.IsMovementAllowed())
            RotateWithKeys();
       ...
       if (moveDir.x == 0 && moveDir.z == 0)
        ...
        ProgressStepCycle(inputDir, walkSpeed);
        return MoveState.WALKING;
    }
    ...
    float GetMaximumSpeedForState(MoveState moveState)
    {
        switch (moveState)
        {
            // idle, walkning, mounted use walkSpeed which is set by Entity
            case MoveState.IDLE:
    ...
    bool WasValidSpeed(Vector3 moveVelocity, MoveState previousState, MoveState nextState, bool combining)
    {
        ...
        // -> we might be going from crouching to walkning or vice versa, so
    ...
    void FixedUpdate()
    {
        ...
        if (isLocalPlayer)
        {
            ...
            else if (state == MoveState.WALKING) state = UpdateWALKING(inputDir, desiredDir);
    ...
    void ProgressStepCycle(Vector3 inputDir, float speed)
    {
        ...
        {
            stepCycle += (controller.velocity.magnitude + (speed * walkStepLength)) * Time.fixedDeltaTime;
        }

        if (stepCycle > nextStep)
        {
            nextStep = stepCycle + walkStepInterval;
            PlayFootStepAudio();
        }
    }
    ...
    void UpdateAnimations()
    {
        ...
        {
            animator.SetFloat("DirX", localVelocity.x, directionDampening, Time.deltaTime); // smooth idle<->walk transitions
            animator.SetFloat("DirY", localVelocity.y, directionDampening, Time.deltaTime); // smooth idle<->walk transitions
            animator.SetFloat("DirZ", localVelocity.z, directionDampening, Time.deltaTime); // smooth idle<->walk transitions
    ...
    void UpdateCamera()
    {
        ...
        bool rotatingWithMouse = false;
            ...
            //       near the center.
            //  holding down both buttons is used to walk forward + rotation when
            //  walkning forward. otherwise we can't rotate the character while
            //  walkning forward with both mouse buttons held down)
